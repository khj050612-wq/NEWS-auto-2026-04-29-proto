...
# 2. [UI] 페이지 설정 및 고도화된 스타일 (희진님 피드백 반영)
...
st.markdown("""
    ...
    /* 해시태그 카드 디자인 (숫자 강조, 관련기사 텍스트 구석배치) */
    .keyword-card {
        background-color: #E3F2FD; border: 1px solid #BBDEFB; padding: 12px;
        border-radius: 12px; text-align: center; position: relative; min-height: 110px;
        display: flex; flex-direction: column; justify-content: center;
    }
    .keyword-count-box { position: absolute; top: 8px; right: 10px; color: #1976D2; font-size: 0.7rem; font-weight: 600; }
    .count-num { font-size: 0.9rem; font-weight: 800; color: #0D47A1; }
    .keyword-tag { color: #1565C0; font-weight: bold; font-size: 1.15rem; margin-top: 5px; }
    .keyword-basis { color: #1E88E5; font-size: 0.82rem; font-weight: 500; line-height: 1.3; margin-top: 5px; word-break: keep-all; }
    
    /* 요약 및 제목 스타일 */
    .summary-text { font-size: 0.85rem; color: #666; margin: 15px 0px 10px 5px; }
    ...
    /* 글로벌 저널 버튼 행간 최적화 */
    .journal-box { padding: 5px 0px; border-bottom: 1px solid #f0f0f0; }
    div.stButton > button { height: 24px; padding: 0px 10px; font-size: 10px; border-radius: 4px; }
    </style>
...
def generate_smart_report(title):
    ...
    return {
        ...
        "my_kw": my_kw,
        "example": f"{keyword_name} 실습 시 정확한 분석 절차를 준수한 사례"
    }

def get_basis_text(word):
    bases = {
        "디지털": "병리/검사실 디지털 전환 가속화",
        "AI": "인공지능 판독 보조 솔루션 도입 확대",
        "NGS": "유전체 분석 기반 정밀의료 확산",
        "임상병리사": "보건의료 인력 전문성 강화 논의",
        "검사": "POCT 및 스마트 검사 지침 강화"
    }
    ...
    for i, (word, count) in enumerate(words):
        with kw_cols[i]:
            ...
            <div class="keyword-card">
                <div class="keyword-count-box"><span class="count-num">{count}</span>건 <small>관련기사</small></div>
    ...
    with cr:
        ...
        for fp in data[:10]:
            if not re.search('[가-힣]', fp.clean_title):
                # 글로벌 저널 제목에서 포인트 단어 굵게 처리 (Pathology, Diagnostic 등)
                bold_title = fp.clean_title.replace("Pathology", "**Pathology**").replace("Diagnostic", "**Diagnostic**").replace("Clinical", "**Clinical**")
                st.markdown(f'<div class="journal-box">📄 {bold_title}</div>', unsafe_allow_html=True)
...
with tab_archive:
    ...
    with st.form("archive_form", clear_on_submit=True):
        ...
        txt = st.text_area("경험 메모"); sub = st.form_submit_button("저장")
